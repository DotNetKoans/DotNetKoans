...
public class AboutReflection : Koan
{
	...
	// You can use reflection to create type instances at run time, and to invoke and access them
	// Assemblies contain modules, modules contin types and types contain members.
	...
	public void GettingAType()
	{
		...
		Assert.Equal(typeof(FillMeIn), thisIsAStringType.GetType());
	}
	...
	public void ViewingTheAssemblyOfAnObject()
	{
		...
		Assert.Equal(FILL_ME_IN, a.GetName().Name);
	}
	...
	public void InspectingTheTypesInAnExternalAssembly()
	{
		...
		Assert.Contains(typeof(FillMeIn), types);
	}
	...
	public void ExposingNonPublicConstructor()
	{
		...
		// We don't know for sure how many constructors there are in the Assert class when using reflection so we can be safe and check for null before invoking the first element in the array
		...
		Assert.Equal(typeof(FillMeIn), assertInstance.GetType());
	}
	...
	public void ExposingNonPublicFields()
	{
		...
		Assert.Equal(typeof(FillMeIn), privateFieldValue.GetType());
		Assert.Equal(FILL_ME_IN, privateFieldValue);
	}	
}
